...
class AutoVisitorIrGenerationExtension : IrGenerationExtension {
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        ...
        val transformer = ReplaceFunctionCall(moduleFragment, pluginContext)
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        if (expression.symbol == autoVisitor) {
            ...
            val ctx = CallGeneratorCtx(
                computeData(valueArgument.type) ?: error("invalid visitor"),
                expression,
            )
...
private fun computeData(ofType: IrType): VisitableData? {
...
fun isAcceptMethod(func: IrSimpleFunction, hasVisitor: HasVisitorValue): Boolean {
...
private fun IrTypeParameter.isAnyVariable(): Boolean = superTypes.singleOrNull()?.isNullableAny() ?: false

private fun <E> List<E>.invertTwoIfTrue(condition: Boolean) = if (condition) this[1] to this[0] else this[0] to this[1]
...
//class AutoVisitorClassGenerator(
//    val moduleFragment: IrModuleFragment,
//    val pluginContext: IrPluginContext,
//) : IrElementTransformerVoid()
